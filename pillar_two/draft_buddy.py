...
def draft_buddy_abstract(picks,pick_round,player_df,roster):
    ...
    # Need to figure this out
    if roster.can_draft_QB():
        best_available_QBs = player_df[(player_df['Position'] == 'QB') & (player_df['ADP'] >= cur_pick*.85) & ~player_df['Name'].isin(roster.get_QB())].sort_values(by='Points', ascending=False)
    ...
    if roster.can_draft_RB():
        best_available_RBs = player_df[(player_df['Position'] == 'RB') & (player_df['ADP'] >= cur_pick*.85) & ~player_df['Name'].isin(roster.get_RB())].sort_values(by='Points', ascending=False)
    ...
    if roster.can_draft_WR():
        best_available_WRs = player_df[(player_df['Position'] == 'WR') & (player_df['ADP'] >= cur_pick*.85) & ~player_df['Name'].isin(roster.get_WR())].sort_values(by='Points', ascending=False)
    ...
    if roster.can_draft_TE():
        best_available_TEs = player_df[(player_df['Position'] == 'TE') & (player_df['ADP'] >= cur_pick*.85) & ~player_df['Name'].isin(roster.get_TE())].sort_values(by='Points', ascending=False)
